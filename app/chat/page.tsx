...
function Avatar({ name, avatar, size = 32 }: { name: string; avatar: string; size?: number }) {
  ...
  return (
    <div className="rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-[#1d1d1f]"
      style={{ width: size, height: size, background: avatar || "#FFCB05" }}>
      {name[0]?.toUpperCase()}
    </div>
  );
}
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-6" style={{ background: "#080c14" }}>
        <div className="glass max-w-sm w-full p-8 text-center">
          ...
          <p className="text-white/40 text-sm mb-6">Sign in to join the conversation</p>
          ...
          <Link href="/" className="inline-block mt-2 text-white/30 hover:text-white/60 text-xs transition-all">← Back</Link>
        </div>
      </main>
    );
  }
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07]" style={{ background: "rgba(8,12,20,0.9)", backdropFilter: "blur(16px)" }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-display text-lg font-bold text-white leading-tight">Party Chat</h1>
              <p className="text-white/30 text-xs">as <span style={{ color: "#FFCB05" }}>{name}</span></p>
            </div>
          </div>
        </div>

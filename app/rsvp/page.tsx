...
export default function RSVPPage() {
  ...
  const inputCls = "w-full rounded-2xl px-4 py-3.5 text-[#0d1525] placeholder-black/30 text-sm transition-all bg-white border border-black/10 shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-300/40";

  if (pageStatus === "success") {
    return (
      <main className="min-h-screen flex items-center justify-center px-6 pb-24" style={{ background: "#f4f7ff" }}>
        <div className="max-w-sm w-full text-center">
          <div className="mb-6 p-4 rounded-3xl inline-block" style={{ background: "rgba(255,203,5,0.15)" }}>
            <GraduationCap className="w-16 h-16" style={{ color: "#8A6E00" }} />
          </div>
          <h2 className="text-3xl font-bold mb-3" style={{ color: "#0d1525", letterSpacing: "-0.03em" }}>
  ...
  return (
    <main className="min-h-screen px-4 py-12 pb-24 relative" style={{ background: "#f4f7ff" }}>
      ...
      <div className="relative z-10 max-w-lg mx-auto">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-bold mb-1" style={{ color: "#0d1525", letterSpacing: "-0.03em" }}>
        ...
        {status === "authenticated" && session?.user && (
          <div className="mb-6 flex items-center justify-between px-4 py-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,203,5,0.3)", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            <div className="flex items-center gap-3">
              {session.user.image && <Image src={session.user.image} alt="" width={32} height={32} className="rounded-full" />}
              <div>
                <p className="text-sm font-semibold" style={{ color: "#0d1525" }}>{session.user.name}</p>
                <p className="text-xs" style={{ color: "rgba(0,0,0,0.4)" }}>{isEdit ? "Editing your RSVP" : "New RSVP"}</p>
              </div>
            </div>
        ...
        {pageStatus === "checking" ? (
          ...
          <form onSubmit={submit} className="space-y-4">
            <div className="grid grid-cols-2 gap-2 p-1 rounded-2xl" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
              {([true, false] as const).map(v => (
                <button key={String(v)} type="button" onClick={() => update("attending", v)}
                  ...
                  style={form.attending === v
                    ? { background: v ? "linear-gradient(135deg,#FFCB05,#f5c400)" : "rgba(0,0,0,0.08)", color: v ? "#0d1525" : "#0d1525" }
                ...
                <div className="grid grid-cols-2 gap-3">
                  {([ ["Adults","adults",1], ["Kids","kids",0] ] as [string,string,number][]).map(([label,key,min]) => (
                    <div key={key}>
                      ...
                      <div className="flex items-center justify-between rounded-2xl px-4 py-3" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(0,0,0,0.1)", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                        <button type="button" onClick={() => update(key, Math.max(min, (form[key as keyof typeof form] as number) - 1))}
                          className="text-xl font-light w-10 h-10 rounded-xl flex items-center justify-center transition-all" style={{ color: "#8A6E00" }}>−</button>
                        <span className="font-semibold tabular-nums" style={{ color: "#0d1525" }}>{form[key as keyof typeof form] as number}</span>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      ...
                      <button key={val} type="button" onClick={() => update("diet", val)}
                        ...
                        style={form.diet === val
                          ? { borderColor: "#FFCB05", background: "rgba(255,203,5,0.12)", color: "#8A6E00" }
            ...
            {pageStatus === "error" && <p className="text-red-500 text-xs text-center">Something went wrong. Please try again.</p>}

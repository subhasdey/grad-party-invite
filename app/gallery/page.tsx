...
export default function GalleryPage() {
  ...
  const [isRsvped, setIsRsvped] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    if (status === "authenticated" && userEmail) {
      fetch(`/api/rsvp/check?email=${encodeURIComponent(userEmail)}`)
        .then(r => r.json())
        .then(d => setIsRsvped(isAdmin || d.rsvped))
        .catch(() => setIsRsvped(false));
    }
  }, [status, userEmail, isAdmin]);

  const upload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const uploaderName = userName || guestName.trim();
    if (!uploaderName) { setError("Please enter your name to upload"); return; }
    setError("");
    setUploading(true);
    setProgress(15);
    ...
    try {
      const blob = await blobUpload(file.name, file, {
        ...
        clientPayload: JSON.stringify({ name: uploaderName, caption: caption.trim() }),
      ...
      const mediaRes = await fetch("/api/media", {
        ...
        body: JSON.stringify({ name: uploaderName, url: blob.url, caption: caption.trim(), type, email: userEmail }),
      });
      if (!mediaRes.ok) {
  ...
  const inputCls = "bg-white border border-black/10 rounded-xl px-4 py-3 text-[#0d1525] placeholder-black/30 text-sm w-full shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-300/40";

  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#f4f7ff" }}>

      {/* Nav */}
      <nav className="flex items-center justify-between px-5 md:px-10 py-5 border-b" style={{ borderColor: "rgba(0,0,0,0.08)", background: "rgba(244,247,255,0.95)", backdropFilter: "blur(16px)" }}>
        <div className="flex items-center gap-3">
          <Link href="/" className="text-sm transition-all" style={{ color: "rgba(0,0,0,0.35)" }}>←</Link>
          <h1 className="font-display text-xl font-bold" style={{ color: "#0d1525" }}>Gallery</h1>
        </div>
        <div className="flex gap-2">
          <Link href="/chat" className="px-4 py-2 text-xs rounded-full border transition-all" style={{ color: "rgba(0,0,0,0.45)", borderColor: "rgba(0,0,0,0.12)" }}>Chat</Link>
          <Link href="/rsvp" className="px-4 py-2 text-xs font-semibold rounded-full" style={{ background: "#FFCB05", color: "#0d1525" }}>RSVP</Link>
        </div>
      </nav>
      ...
      <main className="flex-1 px-4 md:px-8 py-8 pb-24 max-w-6xl mx-auto w-full">
        ...
        {/* Thank you banner */}
        <div className="mb-6 px-5 py-4 rounded-2xl text-center" style={{ background: "linear-gradient(135deg, rgba(255,203,5,0.15), rgba(0,39,76,0.08))", border: "1px solid rgba(255,203,5,0.3)" }}>
          <p className="text-sm font-semibold leading-relaxed" style={{ color: "#0d1525" }}>
            🎓 Thank you all for being part of an important milestone of our next generation&apos;s life!
          </p>
        </div>
        ...
        <div className="mb-8 p-5 rounded-2xl" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
          ...
          {status === "unauthenticated" ? (
            <div className="py-2">
              <div className="mb-3">
                <label className="block text-xs mb-1.5" style={{ color: "rgba(0,0,0,0.4)" }}>Your name</label>
                <input value={guestName} onChange={e => setGuestName(e.target.value)} placeholder="Enter your name..." className={inputCls} />
              </div>
              <div className="mb-3">
                <label className="block text-xs mb-1.5" style={{ color: "rgba(0,0,0,0.4)" }}>Caption (optional)</label>
                <input value={caption} onChange={e => setCaption(e.target.value)} placeholder="Add a caption..." className={inputCls} />
              </div>
              {error && <p className="text-red-500 text-xs mb-3">{error}</p>}
              {progress > 0 && (
                <div className="h-1 rounded-full overflow-hidden mb-3" style={{ background: "rgba(0,0,0,0.08)" }}>
              ...
              <button
                ...
                className="flex items-center gap-2.5 px-5 py-3 rounded-xl text-sm font-semibold transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 mb-4"
              ...
              <p className="text-xs" style={{ color: "rgba(0,0,0,0.4)" }}>
                Or{" "}
                <button onClick={() => signIn("google")} className="underline font-medium" style={{ color: "#0d1525" }}>sign in with Google</button>
            ...
            <>
              <div className="flex items-center gap-2 mb-4 mt-1">
                ...
                <span className="text-sm" style={{ color: "rgba(0,0,0,0.5)" }}>Uploading as <span className="font-medium" style={{ color: "#0d1525" }}>{userName}</span></span>
              </div>
        ...
        {/* Grid — RSVPed guests only */}
        {status === "unauthenticated" ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="mb-4 p-4 rounded-3xl inline-block" style={{ background: "rgba(255,203,5,0.12)" }}>
              <svg className="w-10 h-10" fill="none" stroke="#8A6E00" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"/>
              </svg>
            </div>
            <p className="font-semibold mb-1" style={{ color: "#0d1525" }}>Sign in to view the gallery</p>
            <p className="text-sm mb-6" style={{ color: "rgba(0,0,0,0.4)" }}>Gallery is available to RSVPed guests</p>
          ...
          <div className="flex items-center justify-center py-24" style={{ color: "rgba(0,0,0,0.3)" }}>
            <span className="w-5 h-5 border-2 border-current border-t-transparent rounded-full inline-block animate-spin mr-3" />
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="font-semibold mb-1" style={{ color: "#0d1525" }}>RSVP required to view the gallery</p>
            <p className="text-sm mb-6" style={{ color: "rgba(0,0,0,0.4)" }}>Please RSVP to access photos and videos from the celebration</p>
            <a href="/rsvp"
              className="px-6 py-3 rounded-2xl font-semibold text-sm transition-all hover:scale-[1.02]"
              style={{ background: "#FFCB05", color: "#0d1525" }}>
              RSVP Now
            </a>
          </div>
              ...
              return (
                <div key={item._id} className="flex flex-col gap-2">
                  <div
                    ...
                    style={{ aspectRatio: "3/4", background: "rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.06)" }}
                  >
                    {item.type === "image" ? (
                      ...
                      <>
                        <div className="w-full h-full" style={{ background: "linear-gradient(160deg,#e8eeff,#d4dcff)" }} />
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 pointer-events-none">
                          <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ background: "rgba(255,203,5,0.2)", border: "2px solid rgba(255,203,5,0.5)" }}>
                            <svg className="w-6 h-6 ml-1" viewBox="0 0 24 24" fill="#8A6E00"><path d="M8 5v14l11-7z"/></svg>
                          </div>
                          <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(0,0,0,0.4)" }}>Video</span>
                        </div>
                      </>
                    ) : (
                      <video
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent">
                      <button
                        ...
                        {item.likes > 0 && <span className="text-[11px] font-semibold" style={{ color: "#0d1525" }}>{item.likes}</span>}
                      </button>
                    ...
                    editingId === item._id ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white border border-black/10 rounded-xl px-3 py-2 text-[#0d1525] placeholder-black/25 text-xs"
                        />
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="py-2 px-3 rounded-xl text-xs font-medium transition-all disabled:opacity-50 hover:bg-red-50"
      ...
      {/* Lightbox — keep dark for immersive photo viewing */}
      {selected && (
        <div
          ...
          <div className="relative max-w-5xl w-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex-1 flex items-center justify-center overflow-hidden rounded-2xl">
              {selected.type === "image" ? (
                ...
                <div className="flex flex-col items-center gap-4 py-12 px-6 text-center rounded-2xl w-full max-w-sm mx-auto" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}>
                  <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "rgba(255,203,5,0.12)", border: "2px solid rgba(255,203,5,0.3)" }}>
